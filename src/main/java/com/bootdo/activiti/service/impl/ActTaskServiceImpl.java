...
//        vars.put("title","");
     ...
     */
    @Override
    public String startProcessWithProcDefId(String procDefId,String businessId, String title, Map<String, Object> vars) {
        String userId = ShiroUtils.getUser().getUsername();//ObjectUtils.toString(UserUtils.getUser().getId())
    ...
    public String getFormKey(String procDefId, String taskDefKey) {
        ...
        if (StringUtils.isNotBlank(procDefId)) {
            ...
            //如果动态表单没有设置则根据流程配置FlowConfig拿到流程类型后再从常量中拿到类型对应的formKey常量 2020.03.04 xh
            if (StringUtils.isBlank(formKey)) {
                Map<String,Object> map=new HashMap<>();
                map.put("procdefId",procDefId);
                FlowConfigDO flowConfig=flowConfigService.list(map).get(0);
                String flowType=flowConfig.getFlowType();
                formKey=ActivitiConstant.FLOWTYPE_MAPPER.get(flowType);
            }
            if (StringUtils.isBlank(formKey)) {
                formKey = "/404";
            }
