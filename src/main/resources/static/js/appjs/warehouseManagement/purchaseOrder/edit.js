...
function loadDicts() {
	...
	$.ajax({
		url: '/common/dict/list/purchase_state',
		success: function (data) {
			var html = "";
			// 加载数据
			html += "<option value=''>采购状态</option>";
			for (var i = 0; i < data.length; i++) {
				html += '<option value="' + data[i].value + '">' + data[i].name + '</option>'
			}
			$("#purchaseState").append(html);
			$("#purchaseState").chosen({
				maxHeight: 200
			});
			$("#purchaseState").val($("#oldPurchaseState").val());
			$("#purchaseState").trigger("chosen:updated");
		}
	});
}
//文件上传
...
function update() {
	$.ajax({
		cache : true,
		type : "POST",
		url : "/warehouseManagement/purchaseOrder/update",
