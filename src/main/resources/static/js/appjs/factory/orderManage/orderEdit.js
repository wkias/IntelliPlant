...
function update() {
	...
	$.ajax({
		cache : true,
		type : "POST",
		url : "/factory/orderManage/update",
...
function getTotal(t) {
   var price =  $(t).parent().parent().parent().find("[name='price']").val();

   $(t).parent().parent().parent().find("[name='noBatchOrderContentS[][totalSum]']").val($(t).val()*price);
    let sum = 0 ;
    $("[name='noBatchOrderContentS[][totalSum]']").each(function (index) {
...
function loadProduct(product, isBatch) {
    ...
    if ($("[name='noBatchOrderContentS[][productId]']:eq(0)").val() == "") {
        //隐藏域
        $("[name='noBatchOrderContentS[][productId]']:eq(0)").val(product.productId);
        ...
        //隐藏域
        $("[name='noBatchOrderContentS[][productId]']:last").val(product.productId);
        // $("[name='"+eleName+"ContactPersonId']:eq(0)").val(trader.mainContactPerson);
        //b标签文本
        let $text=$("[name='"+eleName+"Item']:eq(0) b");
...
function loadDicts(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#contractType").val($("#oldContractType").val());
