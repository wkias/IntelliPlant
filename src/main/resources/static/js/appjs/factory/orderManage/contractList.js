...
function select(row) {
  ...
  // 获取窗口索引
    ...
    layerParent.document.getElementById("contractId").setAttribute("value", row.contractId);
    layerParent.document.getElementById("contractCode").setAttribute("value", row.contractCode);
    layerParent.document.getElementById("contractName").setAttribute("value", row.contractName);
    layerParent.document.getElementById("contractType").setAttribute("value", row.typeName);
    // $.post("/factory/orderManage/getMaxPeriod",{contractId:row.contractId},function(result){
    //     parent.document.getElementById("orderPeriod").setAttribute("value", result);
    // });
    $.ajax({
        async:false,
        url: "/factory/orderManage/getMaxPeriod",
        type: "get",
        data: {contractId:row.contractId},
        dateType:"text",
        success: function (r) {
            layerParent.document.getElementById("orderPeriod").setAttribute("value", r);
        }
    });
...
function add() {
    ...
    // });
    window.open( prefix + '/add','新增合同','location=no')
}
function details(id){
    // layer.open({
    //     type:2,
    ...
    // })
    window.open( prefix + '/details/'+id,"合同详情","location=no")
}
function edit(id) {
    // layer.open({
    //     type: 2,
    ...
    // });
    window.open( prefix + '/edit/'+id,"编辑合同","location=no")
}
